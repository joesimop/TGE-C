...
void create_frame_buffers(VulkanCore* core) {
    ...
    for (size_t i = 0; i < core->swapChainImageCount; i++) {
        VkImageView* attchments = &core->swapChainImageViews[i];

        VkFramebufferCreateInfo frameBufferInfo;
        frameBufferInfo.sType = VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;
        frameBufferInfo.renderPass = core->renderPass;
        frameBufferInfo.attachmentCount = 1;
        frameBufferInfo.pAttachments = attchments;
        ...
        ASSERT(vkCreateFramebuffer(core->logicalDevice, &frameBufferInfo, NULL, &core->swapChainFramebuffers[i]) == VK_SUCCESS, "Failed to create frame buffer");
    }
